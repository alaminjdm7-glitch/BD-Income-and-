...
<head>
    ...
    <title>VIP Earning App V6.0</title>
    ...
    <style>
        ...
        #maintenance-screen {
            display: none;
            position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: var(--white); z-index: 99998;
        ...
        .maintenance-msg { font-size: 14px; color: #666; line-height: 1.6; }
        .timer-badge {
            display: inline-block; margin-top: 20px; padding: 10px 20px;
            background: #ffebee; color: var(--danger);
            border-radius: 30px; font-weight: bold; font-family: monospace; font-size: 16px;
        }
        ...
        .support-float {
            ...
            z-index: 900; font-size: 28px; cursor: pointer;
        ...
        .u-avatar { 
            width: 52px; height: 52px; background: rgba(255,255,255,0.2); backdrop-filter: blur(5px);
            border-radius: 50%; display: flex; align-items: center; justify-content: center; 
            font-size: 22px; border: 2px solid rgba(255,255,255,0.4); position: relative;
        }
        /* VERIFICATION BADGES */
        ...
        .balance-card {
            ...
            margin: -70px 20px 0; padding: 25px;
        ...
        .vip-banner {
            ...
            background: linear-gradient(45deg, #1e272e, #000000);
            border-radius: 20px; color: #d4af37;
            display: flex; justify-content: space-between; align-items: center;
            box-shadow: 0 5px 15px rgba(0,0,0,0.2); border: 1px solid #d4af37;
        }
        ...
        /* PLAN ITEM */
        .plan-box {
            background: white; margin: 0 20px 15px; padding: 20px;
            border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.03);
            border: 1px solid #f0f0f0; display: flex; gap: 15px;
        }
        ...
        .btn-buy {
            ...
            padding: 8px 16px; border-radius: 20px; font-weight: 700; font-size: 12px; cursor: pointer;
        }
        ...
        .btn-pro {
            ...
            padding: 10px 20px; border-radius: 30px; font-weight: 700; font-size: 12px; cursor: pointer;
            box-shadow: 0 4px 10px rgba(108, 92, 231, 0.3);
        }
        .btn-pro:disabled { background: #ccc; box-shadow: none; }
        ...
        .rules-box {
            background: white; margin: 0 20px 20px; padding: 20px; border-radius: 20px;
        ...
        .verify-card {
            ...
            color: white; padding: 20px; margin: 0 20px 20px;
            border-radius: 20px; text-align: center;
        }
        .verify-card.unverified { background: linear-gradient(135deg, #636e72, #b2bec3); }

        /* BOTTOM NAV */
        .btm-nav {
            position: fixed; bottom: 20px; left: 20px; right: 20px;
            background: #2d3436; padding: 15px 30px;
            display: flex; justify-content: space-between;
            border-radius: 25px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); z-index: 1000;
        }
        ...
        input, select { width: 100%; padding: 14px; border: 1px solid #eee; border-radius: 12px; margin-bottom: 12px; background: #f9f9f9; box-sizing: border-box; outline: none; }
        ...
        /* Runtime Timer Style */
        .runtime-strip {
            background: #222; color: #0f0; font-size: 11px; padding: 5px 20px; text-align: center; font-family: monospace; font-weight: bold;
        }
...
<body>
    ...
    <div id="loader-screen">
        ...
        <div class="app-loading-text">INITIALIZING V6.0...</div>
    </div>
    ...
    <div id="maintenance-screen">
        ...
        <div class="maintenance-msg" id="maint-msg">We are currently under maintenance. Please check back later.</div>
    ...
    <div id="main-app">
        ...
        <div id="popup-notice">
            <div class="popup-card">
                ...
                <button onclick="document.getElementById('popup-notice').style.display='none'" style="background:var(--primary); color:white; border:none; padding:12px 30px; border-radius:30px; font-weight:bold; margin-top:20px; cursor:pointer;">Okay, I Understand</button>
            </div>
        </div>
        ...
        <div class="vip-banner" onclick="go('p-vip', document.querySelectorAll('.nav-it')[1])">
            <div>
                ...
                <div style="font-size:11px; opacity:0.8;">Get verified & earn daily income</div>
            </div>
        ...
        <div id="p-vip" class="page">
            ...
            <!-- VERIFICATION STATUS -->
            <div id="verify-status-box" class="verify-card unverified">
                <i class="fas fa-user-clock" style="font-size:30px; margin-bottom:10px;"></i>
                <h3 style="margin:0;">Unverified</h3>
                <p style="font-size:12px; margin:5px 0;">Deposit to verify & unlock withdrawals.</p>
            </div>
        ...
        <div id="dep-modal" class="modal-ol">
            <div class="modal-in">
                ...
                <div id="dep-info" style="background:#f0f0f0; padding:10px; border-radius:8px; font-size:12px; margin-bottom:10px;">Loading info...</div>
                <input type="number" id="dep-amount" placeholder="Amount">
                <input type="text" id="dep-trx" placeholder="Transaction ID (TrxID)">
                <input type="number" id="dep-num" placeholder="Sent From Number">
    ...
    <script>
        ...
        function init() {
            ...
            db.ref('app_settings').on('value', s => {
                ...
                if(d) {
                    if(d.appName) {
                        ...
                        document.querySelector('.app-loading-text').innerText = d.appName.toUpperCase();
                    }
                    checkMaintenance(d);
                    ...
                    if(d.banner) { document.getElementById('banner-area').style.display='block'; document.getElementById('banner-area').src=d.banner; } // Assuming banner logic exists if needed
                    ...
                    // Deposit Info
                    if(d.depositNumber) document.getElementById('dep-info').innerHTML = `Send Money to: <b>${d.depositNumber}</b><br>Method: ${d.depositMethod || 'Bkash/Nagad'}`;
                }
            });
            ...
            ref.once('value', s => { 
                if(!s.exists()) ref.set({ 
                    ...
                    isVerified: false, verifyType: null, activePlan: null, planExpiry: 0 
                }); 
            });
            ...
            ref.on('value', s => {
                ...
                // Verification Logic
                const vBox = document.getElementById('verify-status-box');
                const vBadge = document.getElementById('u-verified-badge');
                const vTag = document.getElementById('u-vip-tag');

                if(u.isVerified) {
                    vBox.className = 'verify-card';
                    vBox.innerHTML = `<i class="fas fa-check-circle" style="font-size:30px; margin-bottom:10px;"></i><h3 style="margin:0;">Verified Member</h3><p style="font-size:12px; margin:5px 0;">${u.verifyMsg || 'Thank you for joining.'}</p>`;
                    ...
                    vBox.className = 'verify-card unverified';
                    vBox.innerHTML = `<i class="fas fa-user-clock" style="font-size:30px; margin-bottom:10px;"></i><h3 style="margin:0;">Unverified</h3><p style="font-size:12px; margin:5px 0;">Deposit to verify & unlock withdrawals.</p>`;
                    vBadge.innerHTML = '';
                ...
                if(u.activePlan && u.planExpiry > Date.now()) {
                    startPlanTimer(u.activePlan, u.planExpiry);
                } else {
                    stopPlanTimer();
                }
            });
            ...
            db.ref('plans').on('value', s => {
                ...
                Object.keys(plans).forEach(k => {
                    ...
                    <div class="plan-box">
                        ...
                        <button class="btn-buy" onclick="buyPlan('${k}')">BUY</button>
                    </div>`;
                });
            });
        }
        ...
        function buyPlan(planId) {
            db.ref('users/'+user.id).once('value', s => {
                const u = s.val();
                if(!u.isVerified) return alert("❌ You must be verified (Deposit) to buy a plan.");
                if(u.activePlan) return alert("❌ You already have an active plan.");

                db.ref('plans/'+planId).once('value', ps => {
                    const p = ps.val();
                    if(u.balance < p.price) return alert("❌ Insufficient Balance. Please Deposit.");
                    ...
                    if(confirm(`Buy ${p.name} for ৳${p.price}?`)) {
                        ...
                        alert("✅ Plan Activated! You will earn daily automatically.");
                    }
                });
            });
        }

        function startPlanTimer(planId, expiry) {
            if(planInterval) clearInterval(planInterval);
            
            // Auto Income Logic (Client side check mostly, Admin payout usually better but we simulate here)
            planInterval = setInterval(() => {
                ...
                // Simple logic: Add income every 24 hours if last claim > 24h
                // For this demo, we will show timer logic only
                // In a real app, use Cloud Functions for accuracy
        ...
        function reqDep() {
            ...
            const rid = Date.now();
            const d = { 
                reqId: rid, userId: user.id, userName: user.first_name, type: 'Deposit', 
                amount: amt, trxId: trx, number: num, status: 'Pending', 
            ...
            if(settings.maintenance && settings.maintenance.active === true) {
                ...
                if(settings.maintenance.endTime) {
                    ...
                    maintInterval = setInterval(() => {
                        const now = Date.now();
                        const end = parseInt(settings.maintenance.endTime);
                        const diff = end - now;

                        if(diff <= 0) {
                            timerDiv.style.display = 'none';
                            maintDiv.style.display = 'none';
                            mainDiv.style.display = 'block';
                            db.ref('app_settings/maintenance').update({ active: false });
        ...
        function loadPaymentMethods() {
            db.ref('app_settings/paymentMethods').once('value', s => {
                ...
                if(methods) {
                    ...
                    div.innerHTML = '<span style="font-size:12px; color:#aaa;">No methods set</span>';
                }
            });
        }

        // --- CHECK BALANCE & OPEN WD ---
        function checkBalanceAndOpenWd() {
            db.ref('users/'+user.id).once('value', s => {
                ...
                const bal = s.val() || 0;
                if(bal < minWd) {
                    alert(`⚠️ Warning: You need at least ৳${minWd} to withdraw. Current balance: ৳${bal.toFixed(2)}`);
                } else {
        ...
        // --- TASKS ---
        function loadTasks() {
            db.ref('users/'+user.id).once('value', uSnap => {
                const u = uSnap.val();
                const isVerified = u && u.isVerified;
                ...
                db.ref('tasks').on('value', s => {
                    ...
                    Object.keys(s.val()).forEach(k => {
                        ...
                        // LOCK TASK IF NOT VERIFIED
                        let btnHTML = `<button class="btn-pro" onclick="clkTask('${k}','${t.title}',${t.price},'${t.type}')">${btnTxt}</button>`;
                        if(!isVerified) {
                            btnHTML = `<button class="btn-pro" disabled onclick="alert('Verify account first')">🔒 Locked</button>`;
                        }

                        l.innerHTML += `
                        <div class="task-box">
                            <div><div style="font-weight:700; font-size:15px;">${t.title}</div><div style="font-size:11px; color:#888;">${t.desc}</div></div>
                            <div style="text-align:right;"><div style="color:var(--primary); font-weight:800; font-size:16px;">৳${t.price}</div>${btnHTML}</div>
                        </div>`;
                    });
                });
            });
        ...
        function filterInbox(type, btn) {
            ...
            filtered.forEach(i => {
                if(i.type === 'Notification') {
                    l.innerHTML += `<div class="hist-card" style="border-left-color:#f1c40f"><div style="font-size:10px; color:#aaa;">🔔 Notice • ${i.timestamp}</div><div style="font-weight:600; margin-top:5px;">${i.message}</div></div>`;
                    return;
                }
