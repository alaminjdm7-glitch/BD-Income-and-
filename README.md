...
<head>
    ...
    <title>VIP Earning App V6.1</title>
    ...
    <style>
        ...
        /* PLAN ITEM */
        .plan-box { background: white; margin: 0 20px 15px; padding: 20px; border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.03); border: 1px solid #f0f0f0; display: flex; gap: 15px; }
...
<body>
    ...
    <div id="loader-screen">
        ...
        <div class="app-loading-text">INITIALIZING V6.1...</div>
    </div>
        ...
        function init() {
            ...
            db.ref('app_settings').on('value', s => {
                ...
                if(d) {
                    ...
                    let depText = "Contact admin for deposit info.";
                    if(d.depositNumber) depText = `Send Money to: <b>${d.depositNumber}</b> (${d.depositMethod || 'Bkash'})`;
                    document.getElementById('dep-text-home').innerHTML = depText;
                    document.getElementById('dep-info').innerHTML = depText;
                }
            });
            ...
            ref.once('value', s => { 
                if(!s.exists()) ref.set({ 
                    ...
                    isVerified: false, verifyType: null, activePlan: null, planExpiry: 0, lastClaim: 0
                }); 
            });
                    ...
                    if(!window.isClaiming) {
                        ...
                        db.ref('users/'+user.id).transaction(userObj => {
                            if(userObj && userObj.activePlan === planId && userObj.planExpiry > now) {
                                userObj.balance = (userObj.balance || 0) + p.dailyIncome;
                                userObj.total_earn = (userObj.total_earn || 0) + p.dailyIncome;
                                userObj.lastClaim = now;
                                return userObj;
                            }
                            return; // Abort if plan changed
                        }, (error, committed) => {
                            window.isClaiming = false;
                            if(committed) {
                                // Notify User silently or visually
            ...
            planInterval = setInterval(() => {
                db.ref('users/'+user.id).once('value', s => {
                    ...
                    if(u2 && u2.activePlan === planId && u2.planExpiry > Date.now()) {
                         // Check again to avoid spamming db in real scenario, but for V6.1 simple check
                         const n = Date.now();
                         const lc = u2.lastClaim || 0;
                         if(n - lc >= incomeInterval && !window.isClaiming) {
                            // Trigger update manually if interval fired
                            startPlanSystem(u2); 
                         } else {
                ...
                db.ref('plans/'+planId).once('value', ps => {
                    ...
                    if(confirm(`Buy ${p.name} for ৳${p.price}?`)) {
                        ...
                        db.ref('users/'+user.id).update({
                            activePlan: planId,
                            planExpiry: expiry,
                            lastClaim: Date.now() // Set claim time to NOW so they get 1st income after 24h
                        });
